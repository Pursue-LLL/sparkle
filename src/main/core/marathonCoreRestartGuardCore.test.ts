...
describe('marathonCoreRestartGuardCore', () => {
  it('blocks when marathon quiesce is active and cursor_conn is zero', () => {
    const decision = shouldBlockMarathonCoreColdRestart(
      { quiesceActive: true, cursorConnectionCount: 0, recentMarathonUserMessageCount: 0, updatedAtMs: 1_000 },
      false,
    )
  ...
  it('blocks at marathon threshold boundary via cursor_conn_active (TIP-1)', () => {
    const decision = shouldBlockMarathonCoreColdRestart(
      { quiesceActive: false, cursorConnectionCount: 12, recentMarathonUserMessageCount: 0, updatedAtMs: 1_000 },
      false,
    )
    assert.equal(decision.blocked, true)
  ...
  it('blocks when any cursor connection is active (TIP-1 — FORCE cannot override)', () => {
    const decision = shouldBlockMarathonCoreColdRestart(
      { quiesceActive: false, cursorConnectionCount: 4, recentMarathonUserMessageCount: 0, updatedAtMs: 1_000 },
      false,
    )
    assert.equal(decision.blocked, true)
    assert.equal(decision.reason, 'cursor_conn_active')
  ...
  it('allows cold restart only when cursor_conn is zero and idle', () => {
    const decision = shouldBlockMarathonCoreColdRestart(
      { quiesceActive: false, cursorConnectionCount: 0, recentMarathonUserMessageCount: 0, updatedAtMs: 1_000 },
      false,
    )
  ...
  it('honors SPARKLE_FORCE_CORE_RESTART only when cursor_conn is zero', () => {
    ...
    const blocked = shouldBlockMarathonCoreColdRestart(
      { quiesceActive: true, cursorConnectionCount: 20, recentMarathonUserMessageCount: 0, updatedAtMs: 1_000 },
      true,
    )
  ...
  it('blocks when recent userMessage segments exist even if cursor_conn is zero (P10b)', () => {
    const decision = shouldBlockMarathonCoreColdRestart(
      {
        quiesceActive: false,
        cursorConnectionCount: 0,
        recentMarathonUserMessageCount: 4,
        updatedAtMs: 1_000,
      },
      false,
    )
    assert.equal(decision.blocked, true)
    assert.equal(decision.reason, 'recent_marathon_user_message')
  })
