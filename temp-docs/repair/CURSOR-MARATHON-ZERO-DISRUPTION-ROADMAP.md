...
> **最后更新**：2026-07-28 · 合并 c69260ad 定责、BUG-021、P22 段轮换、Latency Tax、网络面加固 · **Sparkle 仓 Marathon/L7/P21–P23 唯一 SSOT**（open-perplexity 仅薄索引）
...
| | 10:52 A 案 | 14:47–14:50 B 案 | 15:41 C 案 |
|---|---|---|---|
| RID | （多路） | c69260ad / cc6c19f8 / 11b777ba | **c8346504 / 6c2791ff**（同簇） |
| 层 | L2 Sparkle restart | **L7** 长段 generation-ended | **L3** mass PING timeout |
| App | Cursor-2 | Cursor-3 + Cursor-2 | Cursor-3 |
| 并行形态 | 多路同断 | 单路断、其余续流 | **多路同秒** PING code=14 |
| 错误形态 | ECONNRESET / aborted | silent_generation_end · generation-ended | ConnectError `[unavailable] PING timed out` |
| 修复方向 | P10/P23 禁 force install | **P22 段轮换** + P21 | **§29 P19 rescue** · VPS QUIC（**非 P22**） |
| 定责 SSOT | 本文 §10.2 | 本文 §10.1–§10.1c | **Split-Brain SSOT §29.10** |

**铁律**：B 案（L7）与 C 案（L3）**禁止互用修复方案** — P22 不能治 PING timeout；§29 rescue 不能阻止 L7 90min 关流。
...
## 11. 500 Included 终极加固（P21–P22 · 观测面复活）
...
### 11.4 与 Connect Split-Brain SSOT 关系
...
| P1 | **P15 pulse 依赖 P21 验收** | Sparkle | connect_path 分裂脑早检 |
...
## 15. 实施顺序（2026-07-28 起）

1. **立即**：P21 部署（rm `.DS_Store` + pkg **1.26.85**）— 用户确认后执行
2. **Soak 24h**：零 ENOTDIR · rescue executed · jsonl 写入恢复
3. **P22 设计与 IFM 补丁** — 段轮换 @ ~85min
4. **并行推进 §5 Phase 1–4** — Zero-Disruption kernel（长期最大收益）
5. **P23 + Latency Tax UI** — 升级 gate + 双轨告警
6. **Cross-仓**：Guard ghost 拦截与 P22 handoff 联合验收

**完成定义（增量）**：c69260ad 类长段 silent EOF 在 P22 后 **0 复现**；BUG-021 类 observability dead **0 复现**；latency tax 持续 >150ms 必告警且可 triage。
...
| **R-08** | P15 connect_path_pulse soak（post-P21） | 草案 | Connect split-brain 早检 · 不切节点 | `cursorConnectStreamKeepaliveCore.ts` · `marathonTransportDialOrchestratorCore.ts` | Sparkle BUG-016 df1501ed · open-perplexity Split-Brain SSOT §29 | P21 前 pulse/rescue 全灭 |
...
| 文档 | 职责 |
|---|---|
| **`CURSOR-MARATHON-ZERO-DISRUPTION-ROADMAP.md`** | **本文（Sparkle 仓）** — 500 马拉松零中断 + L7 定责 + P21–P23 + P22 + Latency Tax + TODO R-01–R-11 |
...
| 6 | 重启后验收 | ⏸ **等你** | app-log 见 `[LogDiscoveryHealth] outcome=ok` · 零 ENOTDIR · rescue executed |

**你告诉我「可以重启」时**，只执行：`bun run upgrade:mac`（需 cursor_conn=0）+ 验收 3 条。
